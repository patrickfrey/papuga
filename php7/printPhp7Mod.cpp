		...
		const papuga_MethodDescription& method)
{
	...
	out << fmt::format( papuga::cppCodeSnippet( 0,
		...
		"if (!papuga_php_set_CallArgs( &argstruct, (void*)obj, argc, &g_class_entry_map))",
		"{",
			"papuga_destroy_CallArgs( &argstruct);",
			"PHP_FAIL( papuga_ErrorCode_tostring( argstruct.errcode));",
			"return;",
		"}",
		"papuga_init_CallResult( &retstruct, membuf_retv, sizeof(membuf_retv), membuf_err, sizeof(membuf_err));",
		...
		const papuga_ClassDescription& classdef)
{
	...
	out << fmt::format( papuga::cppCodeSnippet( 0,
		...
		"",
		"papuga_init_CallArgs( &argstruct, membuf_args, sizeof(membuf_args));",
		"if (!papuga_php_set_CallArgs( &argstruct,  NULL/*self*/, argc, &g_class_entry_map))",
		"{",
			"papuga_destroy_CallArgs( &argstruct);",
			"PHP_FAIL( papuga_ErrorCode_tostring( argstruct.errcode));",
			"return;",
		...
		const papuga_InterfaceDescription& descr)
{
	...
	out << fmt::format( papuga::cppCodeSnippet( 0,
		...
		"{",
			"fprintf( stderr, \"Shutdown module %s\\n\", \"{modulename}\");",
		"return SUCCESS;",
		"}",
		"PHP_RSHUTDOWN_FUNCTION({modulename})",
		"{",
			"fprintf( stderr, \"Shutdown request %s\\n\", \"{modulename}\");",
			"return SUCCESS;",
		"}",
