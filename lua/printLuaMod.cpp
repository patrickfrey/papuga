		...
		const papuga_MethodDescription& method)
{
	...
	out << fmt::format( papuga::cppCodeSnippet( 0,
		...
		"char errbuf[ 2048];",
		"if (!papuga_lua_init_CallArgs( &arg, ls, lua_gettop(ls), {selfname})) papuga_lua_error( ls, \"{nsclassname}.{methodname}\", arg.errcode);",
		...
		const papuga_ClassDescription& classdef)
{
	...
	out << fmt::format( papuga::cppCodeSnippet( 0,
		...
		"if (!papuga_lua_init_CallArgs( &arg, ls, lua_gettop(ls)-1, NULL)) papuga_lua_error( ls, \"{nsclassname}.new\", arg.errcode);",
