		...
		const papuga_ClassDescription& classdef)
{
	...
	out << fmt::format( papuga::cppCodeSnippet( 0,
		...
		"papuga_init_CallArgs( &arg, membuf_args, sizeof(membuf_args));",
		"if (!papuga_lua_set_CallArgs( &arg, ls, lua_gettop(ls), NULL))",
		"{",
			"papuga_destroy_CallArgs( &arg);",
		...
		"hobj = papuga_alloc_HostObject( {classid}, objref, {destructor});",
		"if (!hobj) goto ERROR_CALL;",
		"return papuga_lua_push_hostobj( ls, \"{nsclassname}\", hobj);",
		"",
		"ERROR_CALL:",
		"if (objref) {destructor}( objref);",
